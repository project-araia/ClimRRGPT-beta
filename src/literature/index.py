...
from src.utils import DATA_DIR, REPO_ROOT, get_file_list
...
parser = argparse.ArgumentParser(
    description="Build USearch index from sectionized JSON files."
)
parser.add_argument(
    "--data_dir",
    type=str,
    default=str(DATA_DIR),
    help="Path to the directory containing sectionized JSON files",
)
parser.add_argument(
    "--limit", type=int, default=DEFAULT_LIMIT, help="Number of documents to index"
)
...
for batch_start in tqdm(range(0, len(files), BATCH_SIZE)):
    batch_files = files[batch_start : batch_start + BATCH_SIZE]  # noqa: E203
    batch_texts = []
    batch_ids = []
...
print(
    f"Encoded {total_encoded} documents in {(total_encoded + BATCH_SIZE - 1) // BATCH_SIZE} batches."
)
