...
from src.llms import OpenSourceCodingModels, OpenSourceModels, OpenSourceVisionModels
from src.utils import load_config

config = load_config("./src/modules/experience/data_visualization.yml")
dataset_description = load_config("./src/modules/experience/dataset_description.yml")[
    "available_datasets"
]
    ...
    if location:
        ...
        messages = [
            {"role": "system", "content": "You are a helpful assistant."},
            {
                "role": "user",
                "content": (
                    f"Output the latitude and longitude of {location_description}. "
                    "Output a JSON with keys 'latitude' and 'longitude'. Use numbers only. "
                    "Do not use 'E', 'W'. Use this template: ```json```"
                ),
            },
        ]
...
if ("profile_done" not in st.session_state) or not st.session_state.profile_done:
    ...
    "selected_datasets" not in st.session_state
    or len(st.session_state.selected_datasets) == 0
):
    ...
    location_input = st.text_input(
        "Describe the general location:", value=st.session_state.responses["Location"]
    )
    ...
    if location_input:
        ...
        st.write(
            "Choose a set of locations of interest by drawing on the map. "
            "The <span style='color:green'>green</span> cells represent the grid cells "
            "that intersect with the drawn shape.",
    ...
    if st.session_state.all_drawings:
        combined_intersecting_cells = gpd.GeoDataFrame(
            columns=grid_cells_gdf.columns, crs=grid_cells_gdf.crs
        )
        for drawing in st.session_state.all_drawings:
            if drawing["properties"].keys() == {"radius"}:
                ...
                gdf_polygon = gdf_polygon.to_crs(
                    "EPSG:3857"
            ...
            intersecting_cells = gpd.sjoin(
                grid_cells_gdf, gdf_polygon, how="inner", predicate="intersects"
            )
            combined_intersecting_cells = pd.concat(
                [combined_intersecting_cells, intersecting_cells], ignore_index=True
            )
        ...
        st.session_state.crossmodel = st.session_state.crossmodel.drop(
            columns=["index_right"]
        )
        ...
        with col2:
            if st.button("Click Me Twice to Save Drawing and Update Grid Map"):
                ...
                if not same_location(
                    [output["center"]["lat"], output["center"]["lng"]],
            ...
            if st.button("Confirm Location"):
                if st.session_state.crossmodel is None:
                    st.warning(
                        "Please draw a shape on the map to confirm the location."
                    )
                elif output["zoom"] != st.session_state.zoom:
                    st.warning(
                        "It seems like you've changed the zoom level. Please save the drawing again."
                    )
                    ...
                    st.warning(
                        "It seems like you've changed the center of the map. Please save the drawing again."
                    )
                else:
                    st.session_state.location_confirmed = True
                    st.rerun()
else:
    st.session_state.analyze_fn_dict = dispatch_analyze_fn(
    ...
    for dataset in st.session_state.selected_datasets:
        ...
        messages.append(
            {"role": "system", "content": dataset_description[dataset]["description"]}
        )
        ...
        messages.append(
            {
                ...
                "content": (
                    ...
                    f"Profession: {st.session_state.responses['Profession']}\n\n"
                    f"Concern: {st.session_state.responses['Concern']}\n\n"
                    f"Timeline: {st.session_state.responses['Timeline']}\n\n"
                    f"Scope: {st.session_state.responses['Scope']}"
                ),
            }
        )
        messages.append(
            {
                "role": "user",
                "content": "Please provide the analysis of the data based on the prompt above.",
            }
        )
            ...
            if prev_dataset != dataset:
                messages = (
                    ...
                    + [
                        {
                            "role": "system",
                            "content": f"Previous analysis for {prev_dataset}:\n{prev_analysis}",
                        }
                    ]
                    + messages[1:]
                )
        ...
        with col3:
            ...
            if f"response_view_{dataset}" not in st.session_state:
                st.session_state[f"response_view_{dataset}"] = (
                    "init"  # start by showing the new response
                )

            get_ai_analysis = st.button(
                "Generate AI Analysis", key=f"get_ai_analysis_{dataset}"
            )
            if get_ai_analysis:
                ...
                st.session_state[f"old_response_{dataset}"] = (
                    llm_response  # Save the old response immediately
                )
                ...
                #         "Feel free to read now! We will double-check it in the backend "
                #         "with data analysis model for better accuracy...</p>",
                #         unsafe_allow_html=True
                #     )
                ...
                #     # st.markdown(
                #     #     "<p style='font-size:small; font-style:italic;'>"
                #     #     "If needed, an updated analysis will be shown here automatically..."
                #     #     "</p>",
                #     #     unsafe_allow_html=True
                #     # )
                ...
                #     #                     "content": (
                #     #                         f"Update your original analysis given this "
                #     #                         f"additional information: {execution_results} "
                #     #                         "\n\nCheck if there is any contradictory information "
                #     #                         "and if any, fix the original ones. "
                #     #                         "Please keep your original analysis unchanged except "
                #     #                         "for the parts that need to be updated or corrected."
                #     #                         "You must mention everything mentioned in this "
                #     #                         "additional information. "
                #     #                         "Use bold fonts to highlight the updated parts. "
            ...
            if dataset in st.session_state.analysis.keys():
                ...
                if allow_editing == "Edit":
                    num_rows = int(
                        (len(st.session_state.analysis[dataset]) // 50 + 1) * 21
                    )
                    output = st.text_area(
                        "Analysis",
                        value=st.session_state.analysis[dataset],
                        height=num_rows,
                        label_visibility="collapsed",
                    )
    ...
    if len(st.session_state.analysis) == len(st.session_state.selected_datasets):
        with col1:
            st.session_state.generate_summary = st.button(
        ...
        if st.session_state.generate_summary:
            ...
            messages = [
                {
                    "role": "system",
                    "content": "You are a helpful assistant summarizing climate data analyses.",
                },
                {"role": "user", "content": summary_prompt},
            ]

            with st.chat_message("assistant"):
                st.session_state.data_analysis_summary = get_response(
                    messages=messages,
                    stream=True,
