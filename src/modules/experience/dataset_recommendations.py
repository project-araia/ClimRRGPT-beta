...
from src.llms import get_default_llm
from src.modules.experience.util import format_to_json
from src.utils import load_config

st.session_state.config = load_config(
    "src/modules/experience/dataset_recommendations.yml"
)
available_datasets = load_config("./src/modules/experience/dataset_description.yml")[
    "available_datasets"
]
...
def parse_dataset_recommendation(response, valid_keywords=available_datasets.keys()):
    ...
    for entry in parsed_data:
        if all(key in entry for key in ["dataset", "relevance", "explanation"]):
            if entry["dataset"] in valid_keywords and entry["relevance"].lower() in [
                "high",
                "medium",
                "low",
            ]:
...
def get_single_dataset_recommendation(dataset_name, dataset_details, recommendations):
    ...
    dataset_recommendation_instructions = st.session_state.config[
        "dataset_recommendation_instructions"
    ]

    messages[0]["content"] = dataset_recommendation_instructions[0]["content"].format(
    ...
    if len(recommendations) > 0:
        messages.append(
            {
                "role": "assistant",
                "content": dataset_recommendation_instructions[3]["content"].format(
                    keywords=dataset_name,
    ...
    messages.append(
        {
            "role": "assistant",
            "content": dataset_recommendation_instructions[1]["content"].format(
                keywords=dataset_name,
                dataset_details=format_to_json([{dataset_name: dataset_details}]),
            ),
        }
    )
    messages.append(
        {"role": "user", "content": dataset_recommendation_instructions[2]["content"]}
    )
    ...
    for attempt in range(max_retries):
        try:
            ...
            recommendation = parse_dataset_recommendation(
                response, valid_keywords=[dataset_name]
            )
            if recommendation:
                return recommendation
        except Exception:
            st.warning(
                f"Oops! We hit a snag while processing {dataset_name} (Attempt {attempt + 1}/{max_retries}). We're trying again!"
            )

    # If all attempts fail, return a friendly message with the suggestion
    st.error(
        f"We couldn't generate a recommendation for {dataset_name} at this time. Don't worry, though!"
    )
    return [
        {
...
def get_dataset_recommendations():
    available_datasets = load_config(
        "./src/modules/experience/dataset_description.yml"
    )["available_datasets"]
    ...
    for i, (dataset_name, dataset_info) in enumerate(available_datasets.items()):
        ...
        dataset_details = {
            key: dataset_info[key] for key in ["description", "instruction"]
        }
        recommendation = get_single_dataset_recommendation(
            dataset_name, dataset_details, recommendations
        )
    ...
    recommendations = sorted(
        recommendations, key=lambda x: ["High", "Medium", "Low"].index(x["relevance"])
    )
...
if ("profile_done" not in st.session_state) or not st.session_state.profile_done:
    ...
    if (
        st.session_state.dataset_recommendation
        and not st.session_state.ai_analysis_complete
    ):
    ...
    if st.session_state.ai_analysis_complete:
        with col1:
            ...
            st.markdown(
                "Need to go over the dataset descriptions again? Click the `Available Datasets` button on the sidebar. 👈"
            )
            if len(st.session_state.dataset_recommendation) == 0:
                ...
                for entry in st.session_state.dataset_recommendation:
                    with st.expander(
                        entry["dataset"],
                        expanded=(entry["relevance"].lower() == "high"),
                    ):
                        st.markdown(
                            f"**Relevance:** <span style='color:{relevance_colors.get(entry['relevance'], 'black')}'>{entry['relevance']}</span>",
