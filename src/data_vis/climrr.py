...
from src.data_vis.climrr_utils import categorize_fwi, convert_to_dataframe, fwi_color
...
class DataVisualizer(ABC):
    ...
    def map_comparing_period(
        self, crossmodels, df, season="spring", scenario="45", add_legend=True
    ):
        ...
        for i, period in enumerate(periods):
            with cols[i]:
                ...
                st_folium(
                    ...
                    width=450,
                    height=450,
                    key=f"{self.keyword}_{season}_{period}_{scenario}",
                )
        if add_legend:
            self.add_legend()

    def map_comparing_period_by_choosing_season(
        self, crossmodels, df, scenario="45", add_legend=True
    ):
        custom_css = """
            <style>
        ...
        season = st.radio(
            ...
            horizontal=True,
            key=f"{self.keyword}_season",
        )
        self.map_comparing_period(
            crossmodels, df, season, scenario=scenario, add_legend=add_legend
        )

    def map_comparing_season_by_choosing_period(self, crossmodels, df):
        period = st.radio(
            "Select Time Period:",
            self.data_info["periods"],
            horizontal=True,
            key=f"{self.keyword}_period",
        )
        ...
        for i, season in enumerate(seasons):
            with cols[i]:
                ...
                st.caption(season)
                st_folium(
                    m, width=350, height=450, key=f"{self.keyword}_{season}_{period}_2"
                )
        self.add_legend()

    def analyze(self, crossmodels, label=None):
        if label is None:
            label = self.keyword
    ...
    def analyze_seasonal(self, crossmodels, df):
        ...
        if (
            sum(df.columns.str.contains("45")) > 0
            and sum(df.columns.str.contains("85")) > 0
        ):
        ...
        if climate_scenarios:
            ...
            self.map_comparing_period_by_choosing_season(
                crossmodels,
                df[["Crossmodel", "hist"] + columns_45],
                scenario="45",
                add_legend=False,
            )
            st.header("RCP 8.5 Scenario")
            self.map_comparing_period_by_choosing_season(
                crossmodels, df.drop(columns=columns_45), scenario="85"
            )
    ...
    def add_legend(self, label=None):
        if label is None:
            label = "Climate Data"
        ...
        fig.colorbar(
            plt.cm.ScalarMappable(norm=norm, cmap=self.color_scale),
            cax=ax,
    ...
    def create_plots(self, mean, title=None, label=None):
        pass

    def create_plot(self, mean, col_label=None, label_with_metric=None, title=None):
        pass

    @abstractmethod
    def display_results(
        self, mean, std, figs, label=None, label_with_metric=None, metric=None
    ):
        pass

    def get_messages(self, table):
        table = pd.DataFrame(table)
        prompt = self.data_info["prompt"].format(
            table_markdown=table.to_markdown(), table_json=table.to_json()
        )
        messages = [
            {
                "role": "system",
                "content": "You are a helpful assistant that interprets climate data and relates it to specific user goals.",
            },
    ...
    def get_coding_messages(self, table):
        table = pd.DataFrame(table)
        prompt = self.data_info["coding_prompt"].format(
...
class ClimRRSeasonalProjectionsFWI(DataVisualizer):
    ...
    def fwi_color_plt(value):
        fwi_class_colors = {
            ...
            "Very High": (
                ...
                0.0,
                0.5,
            ),  # Even darker orange with 50% transparency
    ...
    def get_map(self, crossmodels, df, period, season="spring", label=None):
        ...
        fwi_df_geo = gpd.GeoDataFrame(
            crossmodels.merge(
                season_fwi_df, left_on="Crossmodel", right_on="Crossmodel"
            )
        )
        ...
        m.add_child(
            folium.features.GeoJson(
                ...
                tooltip=folium.features.GeoJsonTooltip(
                    fields=["Crossmodel", col_name, "class"],
                    aliases=["Crossmodel", "FWI", "class"],
                ),
                style_function=lambda x: {
                    "fillColor": fwi_color(x["properties"][col_name]),
    ...
    def create_plots(self, mean, title=None, label=None):
        ...
        fig1 = px.line(
            table.T,
            title="Mean FWI Across Seasons and Time Periods",
        ...
        fig2 = px.line(
            table,
            title="Mean FWI Values Across Seasons and Time Periods",
    ...
    def display_results(
        ...
        with col2:
            ...
            st.write(
                "These charts illustrate the trends in mean FWI values across seasons and time periods."
            )
        ...
        with col1:
            ...
            std_table = pd.DataFrame(
                std.values.reshape(4, 3),
                columns=self.data_info["periods"],
                index=["spring", "summer", "autumn", "winter"],
            )

            display_table = (
                table.map(lambda x: f"{x:.2f}")
                + " ("
                + std_table.map(lambda x: f"{x:.2f}")
                + ")"
            )
...
class ClimRRAnnualClimateScenarios(DataVisualizer):
    ...
    def get_map(self, crossmodels, df, period, season="spring", label=None):
        if label is None:
            label = "Consecutive Days with No Precipitation"
        columns = [col for col in df.columns if period in col]
        cdnp_df = df[["Crossmodel"] + columns]
        col_name = columns[0]

        cdnp_df_geo = gpd.GeoDataFrame(
            crossmodels.merge(cdnp_df, left_on="Crossmodel", right_on="Crossmodel")
        )
        ...
        m.add_child(
            folium.features.GeoJson(
                cdnp_df_geo,
                tooltip=folium.features.GeoJsonTooltip(
                    fields=["Crossmodel", col_name], aliases=["Crossmodel", label]
                ),
                style_function=lambda x: {
                    "fillColor": self.get_color(x["properties"][col_name]),
                    "color": "black",
                    "weight": 1,
                    "fillOpacity": 0.7,
                },
    ...
    def create_plot(self, mean, col_label=None, label_with_metric=None, title=None):
        if col_label is None:
            col_label = "Consecutive Days with No Precipitation"
        if label_with_metric is None:
            label_with_metric = "Consecutive Days with No Precipitation"
        if title is None:
            title = "Consecutive Days with No Precipitation"
        ...
        mean = pd.concat(
            [
                historical_row_45,
                historical_row_85,
                mean[mean["Scenario"] != "Historical"],
            ]
        )

        # Explicitly set the order of the x-axis (Time Period)
        time_order = ["Historical", "Mid-Century", "End-Century"]
        mean["Time Period"] = pd.Categorical(
            mean["Time Period"], categories=time_order, ordered=True
        )
    ...
    def display_results(
        ...
        fig = figs  # In this class, figs is a single figure
        if label is None:
            label = "consecutive dry days"
        if label_with_metric is None:
            label_with_metric = "Consecutive Dry Days"
        if metric is None:
            metric = "days"
        col1, col2, col3 = st.columns([1, 1, 1])
        ...
        with col2:
            st.plotly_chart(fig, use_container_width=True)
            st.write(
                f"This chart illustrates the trends in {label} across the three time periods."
            )
        ...
        with col1:
            ...
            table_45 = pd.concat(
                [table_hist, table.loc[:, table.columns.str.contains("45")]], axis=1
            )
            table_85 = pd.concat(
                [table_hist, table.loc[:, table.columns.str.contains("85")]], axis=1
            )

            std_table_45 = pd.concat(
                [
                    std_table_hist,
                    std_table.loc[:, std_table.columns.str.contains("45")],
                ],
                axis=1,
            )
            std_table_85 = pd.concat(
                [
                    std_table_hist,
                    std_table.loc[:, std_table.columns.str.contains("85")],
                ],
                axis=1,
            )
            ...
            display_table_45 = (
                table_45.map(lambda x: f"{x:.2f}")
                + " ("
                + std_table_45.map(lambda x: f"{x:.2f}")
                + ")"
            )
            display_table_85 = (
                table_85.map(lambda x: f"{x:.2f}")
                + " ("
                + std_table_85.map(lambda x: f"{x:.2f}")
                + ")"
            )
            ...
            st.write(
                f"These tables present the {label} values for each time period and scenario, with standard deviations in parentheses."
            )
...
class ClimRRAnnualProjectionsPrecipitation(ClimRRAnnualClimateScenarios):
    ...
    def add_legend(self, label=None):
        return super().add_legend(label="Precipitation (mm)")

    def create_plot(
        self,
        mean,
        col_label="Precipitation",
        label_with_metric="Precipitation (mm)",
...
class ClimRRAnnualProjectionsTemperature(ClimRRAnnualClimateScenarios):
    ...
    def get_map(self, crossmodels, df, period, season="spring", label=None):
        return super().get_map(
            crossmodels, df, period, label=f"{self.temp_type} Temperature (°F)"
        )

    def add_legend(self, label=None):
        return super().add_legend(label=f"{self.temp_type} Temperature (°F)")

    def create_plot(self, mean, col_label=None, label_with_metric=None, title=None):
        return super().create_plot(
            mean,
            f"{self.temp_type} Temperature",
            f"{self.temp_type} Temperature (°F)",
            f"Mean {self.temp_type} Temperature",
        )
    ...
    def display_results(
        ...
        return super().display_results(
            ...
            fig,
            f"mean {self.temp_type} temperature",
            f"Mean {self.temp_type} Temperature (°F)",
            "°F",
        )
...
class ClimRRAnnualProjectionsCDNP(ClimRRAnnualClimateScenarios):
    ...
    def get_map(self, crossmodels, df, period, season="spring", label=None):
        return super().get_map(
            crossmodels, df, period, label="Consecutive Days with No Precipitation"
        )

    def add_legend(self, label=None):
        if label is None:
            label = "Consecutive Days with No Precipitation"
        return super().add_legend(label)
    ...
    def display_results(
        ...
        fig = figs
        return super().display_results(
            mean, std, fig, "consecutive dry days", "Consecutive Dry Days", "days"
        )
...
class ClimRRAnnualProjectionsWindSpeed(ClimRRAnnualClimateScenarios):
    ...
    def get_map(self, crossmodels, df, period, season="spring", label=None):
        ...
        windspeed_df_geo = gpd.GeoDataFrame(
            crossmodels.merge(windspeed_df, left_on="Crossmodel", right_on="Crossmodel")
        )
    ...
    def create_plot(self, mean, col_label=None, label_with_metric=None, title=None):
        return super().create_plot(
            mean, "Wind Speed", "Wind Speed (m/s)", "Mean Annual Wind Speed"
        )
...
class ClimRRAnnualProjectionsCoolingDegreeDays(DataVisualizer):
    ...
    def get_map(self, crossmodels, df, period, season="spring", label=None):
        ...
        cdd_df_geo = gpd.GeoDataFrame(
            crossmodels.merge(cdd_df, left_on="Crossmodel", right_on="Crossmodel")
        )
    ...
    def add_legend(self, label=None):
        if label is None:
            label = "Cooling Degree Days"
        return super().add_legend(label=label)
    ...
    def create_plot(self, mean, col_label=None, label_with_metric=None, title=None):
        ...
        fig = px.bar(
            table.T,
            title="Cooling Degree Days Comparison",
            labels={"value": "Cooling Degree Days", "index": "Time Period"},
            color_discrete_sequence=["#FFA500", "#FF4500"],
        )  # Orange for historical, Red-Orange for future
    ...
    def display_results(
        ...
        with col2:
            st.plotly_chart(fig, use_container_width=True)
            st.write(
                "This chart compares Cooling Degree Days between historical data and mid-century projections."
            )
        ...
        with col1:
            ...
            st.dataframe(display_table, use_container_width=True)
            st.write(
                "This table presents the mean annual Cooling Degree Days for historical and mid-century periods, with standard deviations in parentheses."
            )
...
class ClimRRAnnualProjectionsHeatingDegreeDays(DataVisualizer):
    ...
    def get_map(self, crossmodels, df, period, season="spring", label=None):
        ...
        hdd_df_geo = gpd.GeoDataFrame(
            crossmodels.merge(hdd_df, left_on="Crossmodel", right_on="Crossmodel")
        )
    ...
    def create_plot(self, mean, col_label=None, label_with_metric=None, title=None):
        ...
        fig = px.bar(
            table.T,
            title="Heating Degree Days Comparison",
    ...
    def display_results(
        ...
        with col2:
            st.plotly_chart(fig, use_container_width=True)
            st.write(
                "This chart illustrates the trend in Heating Degree Days across the time periods."
            )
        ...
        with col1:
            ...
            st.dataframe(display_table, use_container_width=True)
            st.write(
                "This table presents the mean annual Heating Degree Days for each time period, with standard deviations in parentheses."
            )
...
class ClimRRSeasonalProjections(DataVisualizer):
    ...
    def get_map(self, crossmodels, df, period, season="spring", label=None):
        ...
        temp_df_geo = gpd.GeoDataFrame(
            crossmodels.merge(
                season_temp_df, left_on="Crossmodel", right_on="Crossmodel"
            )
        )
    ...
    def create_plots(self, mean, title=None, label=None):
        ...
        fig1 = px.line(
            table.T,
            title=f"{title} Across Seasons and Time Periods",
        ...
        fig2 = px.line(
            table,
            title=f"{title} Values Across Seasons and Time Periods",
    ...
    def display_results(
        ...
        with col2:
            for fig in figs:
                st.plotly_chart(fig, use_container_width=True)
            st.write(
                f"These charts illustrate the trends in {title} values across seasons and time periods."
            )
        ...
        with col1:
            ...
            st.dataframe(display_table, use_container_width=True)
            st.write(
                f"This table presents the mean {title} values for each season and time period, with standard deviations in parentheses."
            )
...
class ClimRRSeasonalProjectionsTemperature(ClimRRSeasonalProjections):
    ...
    def create_plots(self, mean, title=None, label=None):
        return super().create_plots(
            mean, f"Mean {self.temp_type} Temperature", "Temperature (°F)"
        )
...
class ClimRRDailyProjectionsPrecipitation(ClimRRSeasonalProjections):
    ...
    def get_map(self, crossmodels, df, period, season="spring", label=None):
        return super().get_map(
            crossmodels, df, period, label=f"{self.agg_type} Daily Precipitation (mm)"
        )

    def add_legend(self, label=None):
        return super().add_legend(label=f"{self.agg_type} Daily Precipitation (mm)")

    def create_plots(self, mean, title=None, label=None):
        return super().create_plots(
            mean,
            f"Average {self.agg_type} Daily Precipitation",
    ...
    def display_results(
        ...
        return super().display_results(
            ...
            figs,
            f"Average {self.agg_type} Daily Precipitation",
            f"{self.agg_type} Daily Precipitation",
            "mm",
        )
...
class ClimRRAnnualProjectionsHeatIndex(ClimRRAnnualClimateScenarios):
    ...
    def analyze_annual(self, crossmodels, df):
        ...
        self.map_comparing_period(
            crossmodels, df[["Crossmodel"] + columns_seasonal], scenario="seasonal"
        )
        st.header("# of Summer Days Above Threshold")
        threshold = st.radio(
            "Select a threshold (in °F) for the number of summer days above",
            ["95", "105", "115", "125"],
            horizontal=True,
        )
        ...
        fig.colorbar(
            ...
            cax=ax,
            orientation="horizontal",
            label="# of Summer Days Above Threshold",
        )
    ...
    def get_map(self, crossmodels, df, period, season="spring", label=None):
        if "DayMax" in df.columns:
            return super().get_map(
                crossmodels, df, period, season, label="Summer Daily Max Heat Index"
            )
        elif "SeaMax" in df.columns:
            return super().get_map(
                crossmodels, df, period, season, label="Summer Seasonal Max Heat Index"
            )
        else:
            return super().get_map(
                crossmodels,
    ...
    def create_plot(
        self,
        mean,
        col_label="Heat Index",
        label_with_metric="Heat Index",
        ...
        # Explicitly set the order of the x-axis (Time Period)
        time_order = ["Historical", "Mid-Century", "End-Century"]
        mean_max["Time Period"] = pd.Categorical(
            mean_max["Time Period"], categories=time_order, ordered=True
        )
        mean_above["Time Period"] = pd.Categorical(
            mean_above["Time Period"], categories=time_order, ordered=True
        )
        ...
        fig2 = px.bar(
            ...
            title=f"{title} Across Time Periods",
            labels={
                "# of Summer Days Above Threshold": "# of Summer Days Above Threshold",
    ...
    def display_results_helper(self, table, std_table, scenarios, indices):
        ...
        for scenario in scenarios:
            ...
            std_table_scenario = std_table.loc[
                :, std_table.columns.str.contains(scenario)
            ]
            display_table_scenario = (
                table_scenario.map(lambda x: f"{x:.2f}")
                + " ("
                + std_table_scenario.map(lambda x: f"{x:.2f}")
                + ")"
            )
            display_table_scenario.columns = [
                "Historical",
                "Mid-Century",
                "End-Century",
            ]
    ...
    def display_results(
        ...
        with col1:
            ...
            display_table_heat_index = self.display_results_helper(
                table, std_table, ["DayMax", "SeaMax"], ["Daily Max", "Seasonal Max"]
            )
            ...
            display_table_days_above = self.display_results_helper(
                table,
                std_table,
                ["Day95", "Day105", "Day115", "Day125"],
                ["95 F", "105 F", "115 F", "125 F"],
            )
            st.caption("Number of Summer Days Above Threshold")
